...
RSpec.describe Article::PostsController, type: :controller do
  ...
  describe "GET #index" do
    let!(:posts) { FactoryGirl.create_list(:article_post, 20, :published) }
    subject      { get :index }
  ...
  describe "GET #show" do
    ...
    context 'with a published post' do
      let!(:post) { FactoryGirl.create(:article_post, :published) }
      it          { is_expected.to have_http_status :success }
    end
    ...
    context 'with a future published post' do
      let!(:post) { FactoryGirl.create(:article_post, :future_published) }
      it          { expect { subject }.to raise_error ActiveRecord::RecordNotFound }
    end
  end
